...
**지금은 스파이크다.** 엔진은 스텁(`spike/engine.py`) — 주장 분해·유형 분류·질문 틀만 있고 원문 대조가 없어 등급이 전부 ❓다. 이 저장소의 목적은 셋: ①카카오 봇 세 값 실측 ②웹 한 장과 API 계약 ③Vercel 배포 경로.
...
api/health.py
public/index.html 웹 한 장 (붙여넣기 → 카드 → 되물을 질문 복사)
spike/engine.py   스텁 엔진 (순수 함수)        ← 진짜 엔진이 들어올 자리. audit() 계약은 유지
...
- `api/skill.py`는 **5초 안에 끝나는 동기 경로**만 둔다. 스텁 엔진은 즉시 끝나므로 지금은 문제없다
