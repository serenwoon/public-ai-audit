...
### 복지서비스 원문 — 탐침 (다음 규칙 준비)

다음 규칙은 **한국사회보장정보원_중앙부처복지서비스** 원문 대조다(설계: 볼트 `20_설계/02_감리_복지원문대조_설계`). 원문은 로컬에서 키로 받아 **스냅샷**으로 커밋하고 함께 배포한다 — 실행 중 외부 호출이 없고 키가 프로덕션에 들어가지 않는다. 이 데이터의 이용허락범위는 「제한 없음」(data.go.kr 15090532, 2026-09-23 확인).

파서를 기억으로 짐작해 짜지 않으려고 먼저 **탐침**으로 실제 응답을 받는다(`scripts/probe_welfare.py`, 호출 3회). 엔드포인트 주소는 키 없이 찔러 확인했다 — 목록·상세 둘 다 `401 SERVICE_KEY_IS_NULL`(주소는 맞고 키만 없음), 없는 오퍼레이션은 `400 NO_OPENAPI_SERVICE_ERROR`.

키는 환경변수 `DATA_GO_KR_KEY`로만 읽고 화면·파일에 남기지 않는다. 결과는 `data/welfare/_probe/`(git·Vercel 제외).
...
scripts/probe_welfare.py 복지서비스 API 탐침 (로컬 전용, 키는 환경변수)
tests/            소켓을 막고 도는 시험
